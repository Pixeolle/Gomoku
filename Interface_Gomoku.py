...
class GameBoard(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        snap_x = grid_x * self.cell_size
        snap_y = grid_y * self.cell_size

        seuil = self.cell_size // 4

        if (abs(x - snap_x) <= seuil and abs(y - snap_y) <= seuil and grid_x < self.board.width and grid_y < self.board.height):
